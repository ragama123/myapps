        ...
        if online_file and offline_file and online_primary_key_column and offline_primary_key_column:
            ...
            if online_df is not None and offline_df is not None:
                try:
                    ...
                    # Aggregated data for online table by 'User: Account Name (v182) (evar182)'
                    online_aggregated_data = online_df.groupby(online_primary_key_column, as_index=False).agg({
                        'Orders': 'sum',
                        'Order Value': 'sum',
                        'User: Account Name (v182) (evar182)': 'first'
                    })

                    # Aggregated data for offline table by 'EndCustomerName'
                    offline_aggregated_data = offline_df.groupby(['EndCustomerName', offline_market_column], as_index=False).agg({
                    ...
                    merged_table = pd.merge(online_aggregated_data, offline_aggregated_data, how='outer',
                                           left_on=online_primary_key_column, right_on='EndCustomerName',
                    ...
                    total_offline_orders = offline_aggregated_data['HPOrderNo'].sum()
